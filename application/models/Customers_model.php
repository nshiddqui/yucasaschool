...
class Customers_model extends CI_Model
{
    ...
    private function _get_datatables_query($id = '')
    {
     ...
     private function _get_student_datatables_query($id = '')
    {
    ...
    function get_datatables($id = '')
    {
     ...
     function get_student_datatables($id = '')
    {
        $this->_get_student_datatables_query($id);
    ...
    function student_count_filtered($id = '')
    {
    ...
    public function student_count_all($id = '')
    {
        $this->_get_student_datatables_query();
        $query = $this->db->get();
        return $query->num_rows($id = '');
    ...
    public function student_details($custid)
    {
        $this->db->select('S.*,C.name as class_name');
        $this->db->from('enroll as E');
        $this->db->join('student as S','E.student_id = S.student_id');
        $this->db->join('class as C','C.class_id = E.class_id');
        $this->db->where('S.student_id', $custid);
        $query = $this->db->get();
        return $query->row_array();
    }
    ...
    public function money_student_details($custid)
    {
        $this->db->select('SUM(card_amount) AS credit');
        $this->db->from('student_account');
        $this->db->where('student_id', $custid);
		 $this->db->select('SUM(total) AS debit');
    ...
    public function money_student_summry($custid)
    {
        $this->db->select('*');
        $this->db->from('student_account');
        $this->db->where('student_id', $custid);
    ...
    public function due_student_details($custid){
        $this->db->select('SUM(total) AS total,SUM(pamnt) AS pamnt');
        $this->db->from('geopos_invoices');
        $this->db->where('csd', $custid);
        $query = $this->db->get();
    ...
    {
        $query = $this->db->query("SELECT c.*,p.pc FROM geopos_cust_group AS c LEFT JOIN ( SELECT gid,COUNT(gid) AS pc FROM geopos_customers WHERE (loc=".$this->aauth->get_user()->loc." OR loc=0 ) GROUP BY gid) AS p ON p.gid=c.id");
    ...
    public function trans_count_all($id = '')
    {
     ...
     private function _inv_studnet_datatables_query($id)
    {
        $this->db->from('geopos_invoices');
        $this->db->where('geopos_invoices.csd',$id);
        $this->db->join('student','geopos_invoices.csd = student.student_id', 'left');
    ...
    {
        $this->_inv_studnet_datatables_query($id);
        if ($_POST['length'] != -1)
            $this->db->limit($_POST['length'], $_POST['start']);
        $query = $this->db->get();
        return $query->result();
