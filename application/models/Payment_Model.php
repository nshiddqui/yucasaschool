<?php

if (!defined('BASEPATH'))
    exit('No direct script access allowed');

class Payment_Model extends MY_Model {
    
    function __construct() {
        parent::__construct();
    }

    
    public function get_invoice_amount($invoice_id){
        $this->db->select('I.*, SUM(T.amount) AS paid_amount');
        $this->db->from('invoices AS I');        
        $this->db->join('transactions AS T', 'T.invoice_id = I.id', 'left');
        $this->db->where('I.id', $invoice_id);         
        return $this->db->get()->row(); 
    }
	
	 public function get_single_payment_user($role_id, $user_id) {
       
        if ($role_id == TEACHER) {
            $this->db->select('T.*, SG.*,T.email, T.role_id, T.name AS designation');
            $this->db->from('teacher AS T');
            //$this->db->join('users AS U', 'U.id = T.teacher_id', 'left');  
            $this->db->join('salary_grades AS SG', 'SG.id = T.salary_grade_id', 'left'); 
			$this->db->join('salary_payments AS SP', 'SP.id = T.salary_grade_id', 'left'); 
            $this->db->where('T.teacher_id', $user_id);
            return $this->db->get()->row();
           
        }elseif ($role_id == LIBRARIN) {
            $this->db->select('L.*, SG.*,L.email, L.role_id, L.name AS designation');
            $this->db->from('librarian AS L');
            //$this->db->join('users AS U', 'U.id = T.teacher_id', 'left');  
            $this->db->join('salary_grades AS SG', 'SG.id = L.salary_grade_id', 'left'); 
            $this->db->join('salary_payments AS SP', 'SP.id = L.salary_grade_id', 'left'); 
            $this->db->where('L.librarian_id', $user_id);
            return $this->db->get()->row();
           
        }elseif ($role_id == ACCOUNTANT) {
            $this->db->select('A.*, SG.*,A.email, A.role_id, A.name AS designation');
            $this->db->from('accountant AS A');
            //$this->db->join('users AS U', 'U.id = T.teacher_id', 'left');  
            $this->db->join('salary_grades AS SG', 'SG.id = A.salary_grade_id', 'left'); 
            $this->db->join('salary_payments AS SP', 'SP.id = A.salary_grade_id', 'left'); 
            $this->db->where('A.accountant_id', $user_id);
            return $this->db->get()->row();
           
        }else { 
            
            $this->db->select('E.*, SG.*, U.email, U.role_id, D.name AS designation, U.status AS login_status ');
            $this->db->from('employees AS E');
            $this->db->join('designation_users AS U', 'U.designation_users_id = E.user_id', 'left');
            $this->db->join('designations AS D', 'D.id = E.designation_id', 'left'); 
            $this->db->join('salary_grades AS SG', 'SG.id = E.salary_grade_id', 'left'); 
            $this->db->where('E.user_id', $user_id);
            return $this->db->get()->row();
            
        } 
    }
    
    function duplicate_check($salary_month, $user_id, $id = null,$payment_to){           
           
        if($id){
            $this->db->where_not_in('id', $id);
        }
        $this->db->where('salary_month', $salary_month);
        $this->db->where('user_id', $user_id);
        $this->db->where('payment_to', $payment_to);
        return $this->db->get('salary_payments')->num_rows();            
    }
    
    public function get_payment_list($user_id, $payment_to){
        if ($payment_to == 'driver') {
            $this->db->select('SP.*, SG.grade_name, E.name, E.photo,  U.email, U.role_id, D.name AS designation, U.status AS login_status ');
            $this->db->from('salary_payments AS SP');
            $this->db->join('employees AS E', 'E.user_id = SP.user_id', 'left'); 
            $this->db->join('salary_grades AS SG', 'SG.id = E.salary_grade_id', 'left');            
            $this->db->join('designation_users AS U', 'U.designation_users_id = E.user_id', 'left');
            $this->db->join('designations AS D', 'D.id = E.designation_id', 'left'); 
            if($user_id){
                $this->db->where('SP.user_id', $user_id);
            }
            
            $this->db->where('SP.payment_to', 'driver');
            $this->db->order_by('SP.salary_month', 'ASC');
            return $this->db->get()->result();
            
         }else if   ($payment_to == 'security-gaurd') {
            $this->db->select('SP.*, SG.grade_name, E.name, E.photo,  U.email, U.role_id, D.name AS designation, U.status AS login_status ');
            $this->db->from('salary_payments AS SP');
            $this->db->join('employees AS E', 'E.user_id = SP.user_id', 'left'); 
            $this->db->join('salary_grades AS SG', 'SG.id = E.salary_grade_id', 'left');            
            $this->db->join('designation_users AS U', 'U.designation_users_id = E.user_id', 'left');
            $this->db->join('designations AS D', 'D.id = E.designation_id', 'left'); 
            if($user_id){
                $this->db->where('SP.user_id', $user_id);
            }
            
            $this->db->where('SP.payment_to', 'security-gaurd');
            $this->db->order_by('SP.salary_month', 'ASC');
            return $this->db->get()->result();
            
         }else if   ($payment_to == 'warden') {
            $this->db->select('SP.*, SG.grade_name, E.name, E.photo,  U.email, U.role_id, D.name AS designation, U.status AS login_status ');
            $this->db->from('salary_payments AS SP');
            $this->db->join('employees AS E', 'E.user_id = SP.user_id', 'left'); 
            $this->db->join('salary_grades AS SG', 'SG.id = E.salary_grade_id', 'left');            
            $this->db->join('designation_users AS U', 'U.designation_users_id = E.user_id', 'left');
            $this->db->join('designations AS D', 'D.id = E.designation_id', 'left'); 
            if($user_id){
                $this->db->where('SP.user_id', $user_id);
            }
            
            $this->db->where('SP.payment_to', 'warden');
            $this->db->order_by('SP.salary_month', 'ASC');
            return $this->db->get()->result();
            
         }
		 
		 else if ($payment_to == 'teacher') {

            $this->db->select('SP.*, SG.grade_name, T.name, T.email, T.role_id, T.name AS designation');
            $this->db->from('salary_payments AS SP');
            $this->db->join('teacher AS T', 'T.teacher_id = SP.user_id', 'left'); 
            $this->db->join('salary_grades AS SG', 'SG.id = T.salary_grade_id', 'left');            
           if($user_id){
                $this->db->where('SP.user_id', $user_id);
            }
            
            $this->db->where('SP.payment_to', 'teacher');
            $this->db->order_by('SP.salary_month', 'ASC');
            return $this->db->get()->result();
           
        }else if ($payment_to == 'librarian') {
            $this->db->select('SP.*, SG.grade_name, L.name, L.email, L.role_id, L.name AS designation');
            $this->db->from('salary_payments AS SP');
            $this->db->join('librarian AS L', 'L.librarian_id = SP.user_id', 'left'); 
            $this->db->join('salary_grades AS SG', 'SG.id = L.salary_grade_id', 'left');            
            if($user_id){
             $this->db->where('SP.user_id', $user_id);
            }
            $this->db->where('SP.payment_to', 'librarian');
            $this->db->order_by('SP.salary_month', 'ASC');
            return $this->db->get()->result();
           
        }else if ($payment_to == 'accountant') {
            $this->db->select('SP.*, SG.grade_name, A.name, A.email, A.role_id, A.name AS designation');
            $this->db->from('salary_payments AS SP');
            $this->db->join('accountant AS A', 'A.accountant_id = SP.user_id', 'left'); 
            $this->db->join('salary_grades AS SG', 'SG.id = A.salary_grade_id', 'left');            
            if($user_id){
             $this->db->where('SP.user_id', $user_id);
            }
            $this->db->where('SP.payment_to', 'accountant');
            $this->db->order_by('SP.salary_month', 'ASC');
            return $this->db->get()->result();
           
        }else{
           
            $this->db->select('SP.*, SG.grade_name, T.*, U.email, U.role_id, U.status AS login_status ');
            $this->db->from('salary_payments AS SP');
            $this->db->join('teacher AS T', 'T.teacher_id = SP.user_id', 'left'); 
            $this->db->join('users AS U', 'U.id = T.teacher_id', 'left');  
            $this->db->join('salary_grades AS SG', 'SG.id = T.salary_grade_id', 'left'); 
             if($user_id){
                $this->db->where('SP.user_id', $user_id);
             }
             
             $this->db->where('SP.payment_to', 'teacher'); 
             $this->db->order_by('SP.salary_month', 'ASC');
          //$result=$this->db->get()->result();
		   return $this->db->get()->result();
		  //print_r($result);
             
         }
    }
    
    public function get_single_payment($payment_id, $payment_to) {
        
        if ($payment_to == 'employee') {
            $this->db->select('SP.*, SG.grade_name, E.name, E.photo,  U.email, U.role_id, D.name AS designation, U.status AS login_status ');
            $this->db->from('salary_payments AS SP');
            $this->db->join('employees AS E', 'E.user_id = SP.user_id', 'left'); 
            $this->db->join('salary_grades AS SG', 'SG.id = E.salary_grade_id', 'left');            
            $this->db->join('users AS U', 'U.id = E.user_id', 'left');
            $this->db->join('designations AS D', 'D.id = E.designation_id', 'left');          
            $this->db->where('SP.id', $payment_id);
            $this->db->where('SP.payment_to', 'employee');
           
            return $this->db->get()->row();
            
         }else{
           
            $this->db->select('SP.*, SG.grade_name, T.name, T.designation , U.email, U.role_id, U.status AS login_status ');
            $this->db->from('salary_payments AS SP');
            $this->db->join('teacher AS T', 'T.teacher_id = SP.user_id', 'left'); 
            $this->db->join('users AS U', 'U.id = T.teacher_id', 'left');  
            $this->db->join('salary_grades AS SG', 'SG.id = T.teacher_id', 'left'); 
            $this->db->where('SP.id', $payment_id);             
            $this->db->where('SP.payment_to', 'teacher'); 
           
            return $this->db->get()->row();
             
         }
    }
	
	
}
