...
class Assignment_Individual_Model extends MY_Model {
    ...
    function __construct() {
     ...
     public function get_assignment_list($class_id = null,$section_id = null ){
        ...
        if(is_array($class_id)){
              $this->db->where_in('A.class_id', $class_id);
        }else {
    ...
    public function get_assignments_by_class_and_section_student($class_id="",$section_id = "",$student_id = ""){
        $this->db->select('A.*, S.name AS student_name, Subject.name AS subject,C.name AS class_name');
        $this->db->from('assignments_individual AS A');
        $this->db->join('student AS S', 'S.student_id = A.student_id', 'left');
        $this->db->join('subject AS Subject', 'Subject.subject_id = A.subject_id', 'left');
        $this->db->join('class AS C', 'C.class_id = A.class_id', 'left');
        if($class_id){
             if(is_array($class_id)){
                  $this->db->where_in('A.class_id', $class_id);
        ...
        if($student_id){
            if(is_array($student_id)){
                  $this->db->where_in('A.student_id', $student_id);
            }else {
                 $this->db->where('A.student_id', $student_id);
            }
        }
