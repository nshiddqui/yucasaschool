...
class Login extends CI_Controller {
    ...
    function validate_login() {
      ...
      $query = $this->db->get_where('transport_in', $credential);
      //print_r($query);die;
      if ($query->num_rows() > 0) {
          $row = $query->row();
          //print_r($row);die;
      ...
      // Checking login credential for pre student
      $query = $this->db->get_where('designation_users', $credential);
      if ($query->num_rows() > 0) {
          $row = $query->row();
          ...
          $this->session->set_userdata('login_type', 'designation_users');
