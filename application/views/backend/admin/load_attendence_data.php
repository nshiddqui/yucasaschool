...
#my_table tr.header, #my_table tr:hover {
  ...
  background-color: #f1f1f1;
    ...
    <?php echo get_phrase('students'); ?> <i class="entypo-down-thin"></i> | <?php echo get_phrase('date'); ?> <i class="entypo-right-thin"></i>
                       ...
                       <td style="width:950px;float:left;"><div class="container" style="overflow-x:auto; width:100%"> <table>
                            <thead>
    <?php
...
foreach ($period as $dt) {
    ?>
    <!--<th style="text-align: center;"><?php echo $dt->format("d-m"); ?></th>-->
    <?php
}
 ...
 ?></thead></table></div></td>
                    ...
                    <tr style="height:64px;">
                        ...
                        <?php echo $st_name = @$this->db->get_where('student', array('student_id' => $row['student_id']))->row()->name; ?>
                        </td>
                        <td style="width:950px;float:left;"><div class="container" style="overflow-x:auto; width:100%"> <table class="attendaceoveride">
                            <tbody>
                        <?php
                            $status      = 0;
                            $bus_status  = 0;
                            ...
                            foreach ($period as $dt) {
                                ...
                                $query = $this->db->get_where('attendance', array('section_id' => $section_id, 'class_id' => $class_id,  'timestamp' => $timestamp, 'student_id' => $row['student_id']));
...
//print_r($attendance);
                            ...
                            endforeach;
                        ?>
                        <td style="text-align: center;" onclick="attendance(this)" attendanceid = "<?php echo $attendence_idd;?>" class_id="<?= $class_id ?>" timestamp="<?= $timestamp ?>" year="<? $running_year ?>" section_id="<?= $section_id ?>" student_id="<?= $row['student_id'] ?>" student_name = "<?php echo  $st_name; ?>" busAtt="<?php echo  $bus_status; ?>" gateAtt="<?php echo  $gate_status; ?>" classAtt="<?php echo  $status; ?>" style="width:18%">
                             <?php
                   ...
                   <!-- <td style="text-align: center;"><a href="<?php echo site_url('admin/attendance_report_single_view/' . $class_id . '/' . $section_id . '/' . $from .'/'. $to . '/' . $running_year. '/' . $student_id); ?>"-->
                   <!--class="btn btn-primary" target="_blank">Print</a></td>-->
                <?php endforeach; ?>
   ...
   <!--<center>-->
   <!-- <a href="<?php echo site_url('admin/attendance_report_print_view/' . $class_id . '/' . $section_id . '/'. $from .'/'. $to .'/' . $sessional_year); ?>"-->
