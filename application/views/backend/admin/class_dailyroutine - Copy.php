          ...
          ?>
             ...
             // echo date("H:i A",strtotime('+40 minutes',strtotime('13:30 PM')));
                ...
                for($i = 0;$i < $numberofperiod_sum;$i++){ 
                 ...
                 // echo  $count.'---------['.$timetable_time.' - '.$endtime_.']strtotime('.$minutes_.'strtotime('.$timetable_time.'))'; '.$count.' 'class_id'=>$class_id,'section_id'=>$section_id,
                $class_routine_value =  @$this->db->get_where('class_routine',array('period'=>$i+1,'template_id'=>$template_id,'day'=>$day_,'class_id'=>$class_dt->class_id))->row();                                
            ?>
                <td id = "id<?php echo $k; ?>"  data-day="<?=$j;?>">
                        <input type="hidden" name="period[<?php echo $i;?>][<?php echo $k;?>]" value="<?php echo $i+1 ;?>">
                        <input type="hidden" name="day[<?php echo $i;?>][<?php echo $k;?>]"  value="<?php echo $day_ ;?>">
