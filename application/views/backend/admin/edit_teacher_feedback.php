<?php
    $online_feedback = $this->db->get_where('teacher_feedback', array('id' => $id))->result_array();
	$teacher_id = "";
foreach ( $online_feedback as $row2):

    $tea_id = $row2['teacher_id'];
    $teach_id = explode(',', $tea_id);
?>
...
<div class="container-fluid teacher-feedback">
  ...
  <div class="row">
    <form action="<?php echo site_url()?>/admin/teacher_feedback_by_student/edit" class="form-horizontal form-groups-bordered validate" method="post" accept-charset="utf-8" novalidate="novalidate">
        <div class="col-md-6 ">
                ...
                <div class="panel-body">
                    <div class="form-group">
						...
						<input type="hidden" name="id" value="<?php echo $id ?>"/>
                            <input type="text" class="form-control" name="exam_title" value="<?php echo $row2['title'];?>"data-validate="required" data-message-required="Value Required">
                        </div>
                    </div>
            ...
            foreach ($classes as $row):
			 ...
			 <option value="<?php echo $row['teacher_id'];?>"
                                                <?php if(in_array($row['teacher_id'], $teach_id)) echo 'selected';?>>
                                                    <?php echo $row['name'];?>
                                            </option>
            <?php endforeach; ?>
