<?php
    $online_exam_info = $this->db->get_where('scholarship_online_exam', array('online_exam_id' => $param2))->row();
   ...
   // $section = $this->db->get_where('section', array('section_id' => $online_exam_info->section_id))->row()->name;
    ...
    $questions = $this->db->get_where('scholarship_question_bank', array('online_exam_id' => $param2))->result_array();
    $answers = "answers";
    // calculate total marks
    $total_marks = 0;

    $submitted_answer_script_details = $this->db->get_where('scholarship_online_exam_result', array('online_exam_id' => $param2, 'student_id' => $this->session->userdata('login_user_id')))->row_array();
...
<div style="text-align: center;">
    ...
    <b><?php echo get_phrase('class');?>: <?php echo $class;?> |
...
<div style="margin: 50px 20px 20px 20px;">
    <?php
        ...
        $question_type = $this->crud_model->scholarship_get_question_details_by_id($row['question_bank_id'], 'type');
        $question_title = $this->crud_model->scholarship_get_question_details_by_id($row['question_bank_id'], 'question_title');
        $mark = $this->crud_model->scholarship_get_question_details_by_id($row['question_bank_id'], 'mark');
        $submitted_answer = "";
    ?>
    <div style="height: auto;">
        <div style="width: 95%; float: left;">
             ...
             <?php if ($question_type == 'multiple_choice'):
                 $options_json = $this->crud_model->scholarship_get_question_details_by_id($row['question_bank_id'], 'options');
                 $number_of_options = $this->crud_model->scholarship_get_question_details_by_id($row['question_bank_id'], 'number_of_options');
