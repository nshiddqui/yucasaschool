<hr>
<?php
	$exam_ends_timestamp = strtotime(date('d-M-Y', $this->db->get_where('pre_online_exam', array('online_exam_id' => $online_exam_id))->row()->exam_date)." ".$this->db->get_where('pre_online_exam', array('online_exam_id' => $online_exam_id))->row()->time_end);
	...
	$questions = $this->db->get_where('pre_question_bank', array('online_exam_id' => $online_exam_id))->result_array();
	$total_marks = 0;
...
<form class="" action="<?php echo site_url('pre_student/submit_online_exam/'.$online_exam_id); ?>" method="post" enctype="multipart/form-data" id = "answer_script">
