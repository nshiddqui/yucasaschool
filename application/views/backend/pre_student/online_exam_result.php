...
<div class="row">
	<div class="col-md-12">
		<a href="<?php echo site_url('pre_student/online_exam'); ?>" class="btn btn-<?php echo $data == 'active' ? 'primary' : 'white'; ?>">
			<?php echo get_phrase('active_exams');?>
		</a>
		<a href="<?php echo site_url('pre_student/online_exam_result'); ?>" class="btn btn-<?php echo $data == 'result' ? 'primary' : 'white'; ?>">
		...
		<table class="table table-bordered" id="table_export">
            ...
            <tbody>
            	<?php
                    ...
                    $online_exam_details = $this->db->get_where('pre_online_exam', array('online_exam_id' => $row['online_exam_id']))->row_array();
                    $current_time = time();
                    	...
                    	<td>
                            <?php
							 if ($current_time > $exam_end_time){
								$query = $this->db->get_where('pre_online_exam_result', array('student_id' => $this->session->userdata('login_user_id'), 'online_exam_id' => $row['online_exam_id']));
								if ($query->num_rows() > 0) {
