...
class Student extends MY_Controller
{
    ...
    function invoice($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        $page_data['invoices']   = $this->db->get_where('invoices', array('student_id' => $student_id,'paid_status' => (isset($_GET['title'])?$_GET['title']:'paid')))->result_array();
    ...
    function document($do = '', $document_id = '')
    {
        ...
        $page_data['active_link']  = 'assignment';
 ...
 function get_teachers() {
    ...
    function assignment() {
        ...
        $class_id= $this->db->get_where('enroll', array('student_id' => $this->session->userdata('login_user_id')))->row()->class_id;
         ...
         $this->data['assignments'] = array_merge($this->assignment_individual->get_assignment_list($class_id),
                                        $this->assignment_individual->get_assignments_by_class_and_section_student('','',$this->session->userdata('login_user_id'))
        );
    ...
    private function _upload_assignment() {
        ...
        if ($assignment != "") {
            if ($assignment_type == 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' ||
                    ...
                    $assignment_type == 'application/vnd.ms-office' || $assignment_type == 'application/pdf'|| $assignment_type =  'image/jpeg' || $assignment_type= 'image/png') {
