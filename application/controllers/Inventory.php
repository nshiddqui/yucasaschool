...
class Inventory extends MY_Controller {
    ...
    * ********************************************************** */
    public function index() {
		...
		$this->data['inventory_type']     = $this->inventory->get_list('inventory_type', array(), '', '', '', 'id', 'ASC');
    ...
    public function damaged_report() {
        ...
        //die;
        //$this->data['hostels'] = $this->room->get_list('hostels', array('status' => 1), '', '', '', 'id', 'ASC');
        if(!empty($this->input->post('dateto')) && !empty($this->input->post('datefrm'))){
            $this->inventory->db->where('DATE(asset_warehouse.created_at) <=', $this->input->post('dateto'));
            $this->inventory->db->where('DATE(asset_warehouse.created_at) >=', $this->input->post('datefrm'));
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($insert_id) {
                   ...
                   // create_log('Has been added a room no : '.$data['room_no']. ' of '. $hostel->name );
                    ...
                    success($this->lang->line('insert_success'));
                    redirect('inventory/add_inventory');
                } else {
                    error($this->lang->line('insert_failed'));
                    redirect('inventory/add_inventory');
                }
...
//print_r($this->data['inventory']);
    ...
    public function asset_edit_inv_dmg($id = null) {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($updated) {
                    ...
                    error($this->lang->line('update_failed'));
                    redirect('inventory/asset_edit_inv_dmg/' . $this->input->post('id'));
                }
            } 
        }
        if ($id) {
        ...
        $this->data['edit']       = TRUE;
        $this->data['id']       = $id;
        $this->data['page_name']  = 'edit-asset-dmg-data';
        $this->data['page_title'] = 'Inventory';
        $this->data['folder']     = 'inventory';
        $this->layout->title($this->lang->line('edit') . ' ' . $this->lang->line('room') . ' | ' . SMS);
        $this->load->view('backend/page', $this->data);
    ...
    * ********************************************************** */
    ...
    private function _get_posted_inventory_data() {
        ...
        if ($this->input->post('id')) {
            ...
            $data['section_id'] = $this->input->post('section_ids');
            $data['distributed_to'] = $this->input->post('section_id');
  ...
  function get_section($class_id) {
    ...
    * ********************************************************** */
    ...
    public function asset_delete_inv($id = null) {
        ...
        $data = $this->db->get_where('asset_warehouse', array('id' => $id));
        if(!empty($data)){
            $this->db->where('id',$id);
    ...
    public function add_inventery_type(){
        if ($_POST) {
            if (isset($_POST['name']) && !empty($_POST['name'])) {
                $insert_id = $this->inventory->insert('inventory_type', [
                    'name' => $_POST['name']
                    ]);
                if ($insert_id) {
    ...
    public function delete_asset($id){
        ...
        if(!is_numeric($id)){
          error($this->lang->line('unexpected_error'));
          redirect('inventory/add_asset?title=Add assets of school');
        }
        
        $room = $this->inventory->get_single_inventory('asset_type', array('id' => $id));
        
        if ($this->inventory->delete('asset_type', array('id' => $id))) {
