 ...
 *  http://codecanyon.net/user/Creativeiteme
...
//$this->load->view('backend/index');
    ...
    function student_information($class_id = '')
    {
        ...
        $page_data['running_year']     = $this->year;
    ...
    function get_students() {
        ...
        if($_GET['action']==='inactive') {
            $this->ajaxload->db->where('status' , '0');
        }else if($_GET['action']==='profile'){
            
        }else{
            $this->ajaxload->db->where('status' , '1');
        }
        $totalData = $this->ajaxload->all_students_count();
        $totalFiltered = $totalData;
        if(empty($this->input->post('search')['value'])) {
            if($_GET['action']==='inactive') {
                $this->ajaxload->db->where('student.status' , '0');
            }else  if($_GET['action']==='profile'){
                
            }else{
                $this->ajaxload->db->where('student.status' , '1');
            }
            $books = $this->ajaxload->all_students($limit,$start,$order,$dir);
        }
        else {
            if($_GET['action']==='inactive') {
                $this->ajaxload->db->where('S.status' , '0');
            }else  if($_GET['action']==='profile'){
                
            }else{
                $this->ajaxload->db->where('S.status' , '1');
            }
            ...
            foreach ($books as $row) {
                ...
                $optionss = '<div class="btn-group"><button type="button" class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown">
                        ...
                        if($_GET['action']==='profile'){
                            $options='<a href="'.base_url('/admin/student_profile/'.$row->student_id).'"><i class="entypo-user"></i>View Profile </a>';
                        }else {
                            $options='<div class="btn-group"> <button type="button" class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown">Action <span class="caret"></span> </button>
                                <ul class="dropdown-menu dropdown-default pull-right" role="menu">
                                   <li><a href="'.base_url('/admin/student_marksheet_print_view/'.$row->student_id).'" target="_blank"><i class="entypo-chart-bar"></i>Mark Sheet </a> </li>
                                    <li><a href="'.base_url('/admin/student_profile/'.$row->student_id).'"><i class="entypo-user"></i>Profile </a></li>
    ...
    function student_marksheet_print_view($student_id , $exam_id='') {
     ...
     function student_marksheet_print_view_individual($student_id , $exam_id='') {
        if ($this->session->userdata('admin_login') == 1 || $this->session->userdata('teacher_login') == 1 || $this->session->userdata('parent_login') == 1){
            

        $class_id     = $this->db->get_where('enroll' , array(
            'student_id' => $student_id , 'year' => $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description
        ))->row();
        $class_name   = $this->db->get_where('class' , array('class_id' => $class_id->class_id))->row()->name;

        $page_data['student_id'] =   $student_id;
        $page_data['class_id']   =   $class_id->class_id;
        $page_data['section_id']   =   $class_id->section_id;
        $page_data['exam_id']    =   $exam_id;
        $this->load->view('backend/admin/student_marksheet_print_view_individual', $page_data);
        }else{
            redirect(site_url('login'), 'refresh');
        }
    }
    ...
    function student($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            if($validation == 1) {
                ...
                if($this->input->post('dormitory_id') != null && $this->input->post('hostel_id')  != null && $this->input->post('beds')  != null){
                    $data_hostel['hostel_id']      = $this->input->post('hostel_id');
                    $data_hostel['room_id']        = $this->input->post('dormitory_id');
                    $data_hostel['beds']        = $this->input->post('beds');
                    ...
                    $data_hostel['custom_member_id'] = $this->member->get_custom_id('hostel_members', 'HM');
    ...
    * ********************************************************** */
    ...
    function parent($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        $this->db->select('parent.*,E.class_id as class_id,E.section_id as section_id,S.name as student_name');
        $this->db->join('student AS S', 'S.parent_id = parent.parent_id');
...
//$page_data['parent_list'] = $this->db->get_where('parentss')->result_array();
   ...
   public function staff_salary_unpaid() {
    ...
    {
        if(empty($param1) && $this->session->userdata('teacher_login') == 1 || $this->session->userdata('parent_login') == 1 || $this->session->userdata('student_login') == 1 || $this->session->userdata('role_id') == 13){
    ...
    function get_teachers() {
        if($this->session->userdata('teacher_login') == 1 || $this->session->userdata('parent_login') == 1 || $this->session->userdata('role_id') == 13){
        ...
        if(!empty($teachers)) {
            foreach ($teachers as $row) {
                ...
                if($this->session->userdata('teacher_login') == 1 || $this->session->userdata('parent_login') == 1){
                    $options = '<a href="'.base_url('admin/teacher_profile/'.$row->teacher_id).'"><i class="entypo-eye"></i>&nbsp;'.get_phrase('view profile').'</a>';
                } else if ($this->session->userdata('admin_login') == 1) {
                                            $options = '<div class="btn-group"><button type="button" class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown">
                                    Action <span class="caret"></span></button><ul class="dropdown-menu dropdown-default pull-right" role="menu"><li>
                                    <a href="'.base_url('admin/teacher_profile/'.$row->teacher_id).'"><i class="entypo-pencil"></i>&nbsp;'.get_phrase('profile').'</a></li><li class="divider"></li><li><a href="#" onclick="teacher_edit_modal('.$row->teacher_id.')"><i class="entypo-pencil"></i>&nbsp;'.get_phrase('edit').'</a></li><li class="divider"></li><li><a href="#" onclick="teacher_delete_confirm('.$row->teacher_id.')"><i class="entypo-trash"></i>&nbsp;'.get_phrase('delete').'</a></li></ul></div>';

                } else {
                    $options = 'NA';
                }
$name='<a href="'.base_url('admin/teacher_profile/'.$row->teacher_id).'">'.$row->name.'</a>';
   ...
   /****MANAGE CLASSE   Co-Scholastic Areas*****/    
    ...
    function exam($param1 = '', $param2 = '' , $param3 = '')
    {
        ...
        if ($param1 == 'delete') {
            ...
            $this->db->join('exam As E', 'E.exam_id = EC.exam_id');
        ...
        if ($this->session->userdata('admin_login') == 1 || $this->session->userdata('teacher_login') == 1)
            {
               ...
               $page_data['adminAccount'] = true;
         $teacher_id= $login_role_id_of = $this->session->userdata('teacher_id');
        $page_data['exam_id']    =   $exam_id;
    ...
    function marks_update($exam_id = '' , $class_id = '' , $section_id = '' , $subject_id = '')
    {
        ...
        foreach($marks_of_students as $row) {
            ...
            $mark_total = $this->input->post('max_marks_'.$row['mark_id']);
            $comment = $this->input->post('comment_'.$row['mark_id']);
             $grade_id = $this->input->post('grade_id_'.$row['mark_id']);
    ...
    function marks_get_subject($class_id="",$section="",$subject_id = '')
     {
      $page_data['class_id']   = $class_id;
  ...
  function exam_get_cycle($exam_id="",$id="")
    ...
    * ********************************************************** */ 
     ...
     function school_income()
    {
        ...
        $sum = $this->db->query('SELECT SUM(net_amount) as sum FROM `invoices` WHERE paid_status="paid" ')->result_array(); 
    ...
    function list_advance_pay(){
        $page_data['active_link']  = 'list_advance_pay';
        $page_data['page_name']  =  'list_advance_pay';
        $page_data['page_title'] =  get_phrase('list_advance_pay');
        $this->load->view('backend/index', $page_data);
    }
     ...
     function manage_travel_report()
    {
        ...
        if(!empty($_GET['title'])){
	        $page_data['page_title'] =  $_GET['title'];
	    }else{
     ...
     if($this->session->userdata('admin_login')==1 || $this->session->userdata('teacher_login')==1)
            {
                if(!empty($this->input->post('type'))){
                    if(!empty($this->input->post('datefrm')) && !empty($this->input->post('dateto'))){
                        $this->crud_model->db->where('DATE(R.create_at) <=',$this->input->post('dateto'));
                        $this->crud_model->db->where('DATE(R.create_at) >=',$this->input->post('datefrm'));
                        $this->crud_model->db->where('R.type',$this->input->post('type'));
                        $this->crud_model->db->where('R.room_status','approve');
                        $page_data['data'] = $this->crud_model->get_room_switch_request();
  ...
  function manage_attendance_employee_view_hostel($class_id = '' , $timestamp = '')
      ...
      function teacher_manage_attendance_out_time( $timestamp = '')
    {
       ...
       function attendance_employee_selector()
       {
        ...
        if($data['class_id']!=='' && $data['class_id']!=='all'){
            $this->db->where('designation_id',$data['class_id']);
        }
        $designations_data = $this->db->get('employees')->result_array();
        ...
        $primary_id = "id";
            foreach($designations_data as $row){
                 ...
                 if($query->num_rows() < 1){
                    $attn_data['designation_id']   = $row['designation_id'];
    ...
    function advance_add_pay(){
        ...
        $data['amount']  = $this->input->post('email');
                ...
                $this->session->set_flashdata('flash_message' , get_phrase('Adavance_Payment _sucessfully_Added'));
                    ...
                    $this->session->set_flashdata('error' , get_phrase('Please_enter_designation'));
                }
             redirect(site_url('admin/add_advance_pay/'));
        }
        ...
        if($this->session->userdata('admin_login')==1 || $this->session->userdata('teacher_login')==1 )
{
    ...
    function attendance_report_employee_view($class_id = '', $from = '',$to = '', $sessional_year = '')
     {
         if($this->session->userdata('admin_login')==1 || $this->session->userdata('teacher_login')==1){
    ...
    {   if($this->input->post('class_id') == '' || $this->input->post('sessional_year') == '') {
        ...
        redirect(site_url('admin/attendance_report_employee_view/' . $data['class_id'] . '/' . $data['from'] . '/' .$data['to'] . '/' . $data['sessional_year']), 'refresh');
    }
    ...
    function get_expenses($datefrom = null,$dateto = null, $categoryId = null) {
        if($this->session->userdata('accountant_login')==1){
        ...
        if(!empty($datefrom) && !empty($dateto)){
            $this->ajaxload->datefrm = $datefrom;
             $this->ajaxload->dateto = $dateto;
        }
        if(!empty($categoryId)){
            $this->ajaxload->category_id = $categoryId;
        }
        if(empty($this->input->post('search')['value'])) {
    ...
    function deleteHostelRequest($id){
            $this->db->where('id', $id);
            $this->db->delete('room_change_request');
            $this->session->set_flashdata('flash_message' , get_phrase('data_deleted'));
            redirect(site_url('admin/roomswitch_request'), 'refresh');
    }

    /***MANAGE EVENT / NOTICEBOARD, WILL BE SEEN BY ALL ACCOUNTS DASHBOARD**/
    function noticeboard($param1 = '', $param2 = '', $param3 = '')
    {
        	 if ($this->session->userdata('admin_login') == 1 || $this->session->userdata('teacher_login') == 1|| $this->session->userdata('transport_login') == 1 || $this->session->userdata('accountant_login') == 1 || $this->session->userdata('role_id') == 13 || $this->session->userdata('role_id') == 17){
    ...
    function manage_online_exam($param1 = "", $param2 = ""){
        ...
        if ($param1 == '') {
            ...
            if(!empty($page_data['class_id'])){
                $this->db->where('class_id',$page_data['class_id']);
            }
...
//print_r($request_data);echo '</pre>';exit;        
  ...
  /****Asset Add*****/
   ...
   function add_bulk_asset() {
    ...
    function ajax_updateStaffAttendance(){
        //print_r($this->input->post());
        $data['status'] = $this->input->post('class_att');
        $data['attendence_by'] = 1;
        $id = $this->input->post('attendanceid');
         ...
         if($query->num_rows() < 1){
            $attn_data['status'] = $this->input->post('class_att');
            $attn_data['attendence_by'] = $this->input->post('attendanceid');
            $attn_data['designation_id']   = $this->input->post('designation_id');
            $attn_data['timestamp']  = $this->input->post('timestamp');
            $attn_data['date']  = date('Y-m-d',strtotime($this->input->post('timestamp')));
...
function teacher_profile($teacher_id)
  {
      if(empty($param1) && $this->session->userdata('teacher_login') == 1 || $this->session->userdata('parent_login') == 1){
...
function holidays(){
  ...
  function delete_holiday($id){
        ...
        public function download_excel()
    {
     ...
     {
         if($this->session->userdata('admin_login')==1 || $this->session->userdata('admin_login')==1 || $this->session->userdata('accountant_login')==1){
     ...
     function manage_account_expenses_section_holder($startDate = '', $endDate = '' , $expHeidid = '')
     {
         if( $this->session->userdata('accountant_login')==1){
        ...
        if(!empty($expHeidid)){
            $this->expenditure->db->where('E.expenditure_head_id',$expHeidid);
        }
        $page_data['expenditures'] = $this->expenditure->get_expenditure_list($startDate,$endDate); 
        }
         
        $page_data['datefrm']    = $startDate;
        $page_data['dateto']    = $endDate;
        $page_data['expenditure_head_id'] = $expHeidid;
  ...
  function manage_account_section_unpaid_holder($datefrm = '', $dateto = '')
    ...
    function manage_account_view(){
    // {   if($this->input->post('class_id') == '' || $this->input->post('datefrm') == '' || $this->input->post('dateto') == '') {
    //         $this->session->set_flashdata('error_message' , get_phrase('please_make_sure_employee_and_sessional_year_are_selected'));
    //         redirect(site_url('admin/manage_account_report'), 'refresh');
    //     }
        $page_data['page_name']         = 'manage_account_view';
        $page_data['active_link']  = 'manage_account_view';
        $this->load->view('backend/index', $page_data);
    }
    ...
    {   if($this->input->post('datefrm') == '') {
            $this->session->set_flashdata('error_message' , get_phrase('please_make_sure_start_date_are_selected'));
            redirect(site_url('admin/manage_account_report'), 'refresh');
        }
        if($this->input->post('dateto') == '') {
            $this->session->set_flashdata('error_message' , get_phrase('please_make_sure_end_date_are_selected'));
            redirect(site_url('admin/manage_account_report'), 'refresh');
        }

     
        $data['start_date'] = $this->input->post('datefrm');
        $data['end_date'] = $this->input->post('dateto');
        $data['expenditure_head_id'] = $this->input->post('expenditure_head_id');
        redirect(site_url('admin/manage_account_expenses_section_holder/' . $data['start_date'].'/'.$data['end_date'] .'/' . $data['expenditure_head_id'] ), 'refresh');
    }
    ...
    {   if($this->input->post('datefrm') == '' || $this->input->post('dateto') == '') {
     ...
     public function export_account_expenses_excel()
      {
      $startDate = $this->input->get('datefrm');
      $endDate  = $this->input->get('dateto');

      $expenditure_heads = $this->expenditure->get_list('expenditure_heads', array('status'=> 1));        
        $expenditures = $this->expenditure->get_expenditure_list($startDate,$endDate); 
...
public function hostel_report_staff()
    {
            if(!empty($this->input->post('datefrm')) && !empty($this->input->post('dateto'))){
                if(!empty($this->input->post('designation_id'))){
                    $this->db->where('hostel_members_staff.designation_id',$this->input->post('designation_id'));
                }
                $this->db->where('DATE(hostel_members_staff.created_at) >=',$this->input->post('datefrm'));
                $this->db->where('DATE(hostel_members_staff.created_at) <=',$this->input->post('dateto'));
    ...
    {
        if(!empty($this->input->post('class_id')) && !empty($this->input->post('section_id'))){
            if(!empty($this->input->post('datefrm')) && !empty($this->input->post('dateto'))){
                $this->db->where('enroll.class_id',$this->input->post('class_id'));
                $this->db->where('enroll.section_id',$this->input->post('section_id'));
                $this->db->where('DATE(hostel_members.created_at) >=',$this->input->post('datefrm'));
                $this->db->where('DATE(hostel_members.created_at) <=',$this->input->post('dateto'));
                ...
                $this->db->join('student','student.student_id=hostel_members.user_id');
                $this->db->join('hostels','hostels.id=hostel_members.hostel_id');
                $this->db->join('rooms','rooms.id=hostel_members.room_id');
...
$data=$query->result_array();
    ...
    /* Get list of birthday in table with with some user details*/
    public function birthday_list($class_id = ''){
        if ($this->session->userdata('admin_login') == 1 || $this->session->userdata('teacher_login') == 1 || $this->session->userdata('parent_login') == 1 || $this->session->userdata('student_login') == 1 || $this->session->userdata('accountant_login') == 1 || $this->session->userdata('role_id') == 13 || $this->session->userdata('role_id') == 18 || $this->session->userdata('role_id') == 17)
           {
                if($_GET['user'] === 'teacher'){
                    $page_data['user_type']     = 'teacher';
                    $page_data['page_name']     = 'birthday_list_teacher';
                }else{
                    $page_data['user_type']     = 'student';
                    $page_data['page_name']     = 'birthday_list';
                }
                $page_data['active_link']  = 'birthday_list';
                $page_data['adminAccount']  = true;
                $page_data['status_code']     = '1';
                $page_data['page_title']    = get_phrase('birthday_list'). " - ".get_phrase('class')." : ".
                                            $this->crud_model->get_class_name($class_id);
                $page_data['class_id']  = $class_id;
                $this->load->view('backend/index', $page_data);
           }else{
    ...
    function birthday_list_ajax() {
        if ($this->session->userdata('admin_login') != 1 && $this->session->userdata('teacher_login') != 1  && $this->session->userdata('parent_login') != 1 && $this->session->userdata('accountant_login') != 1 && $this->session->userdata('role_id') != 13 && $this->session->userdata('role_id') != 18 && $this->session->userdata('role_id') != 17)
            redirect(site_url('login'), 'refresh');
        ...
        if(!empty($books)) {
            foreach ($books as $row) {
                ...
                $nestedData['class_id'] = $this->db->get_where('class', array( 'class_id' =>$student_->class_id))->row()->name;
                $nestedData['section_id'] =$this->db->get_where('section', array( 'section_id' =>$student_->section_id))->row()->name;
                $nestedData['birthday'] =    $row->birthday;
                $data[] = $nestedData;
            }
        }
    ...
    public function get_birthday_list_teachers(){
        $columns = array(
            0 => 'teacher_id',
            1 => 'photo',
            2 => 'name',
    ...
    function class_report(){
        $page_data['report_data']=  $this->crud_model->getClassDetails();
        $page_data['page_name']  = 'class_report';
        $page_data['active_link']  = 'class_report';
        $page_data['page_title'] = get_phrase('class_report');
        $this->load->view('backend/index', $page_data);
    }
    
    function staff_birthday_list(){
        $data = array();
        if(!empty($this->input->post('date_from')) && !empty($this->input->post('date_to'))){
            ...
            $this->employee->db->where('DATE_FORMAT(dob,"%m-%d") = "'. date('m-d'). '"');
        }
        if(!empty($this->input->post('designation_id'))){
        ...
        $page_data['adminAccount'] = true;
        $page_data['page_name']  = 'staff_birthday_list';
        $page_data['active_link']  = 'staff_birthday_list';
        $page_data['page_title'] = get_phrase('staff_birthday_list');
        $this->load->view('backend/index', $page_data);
    }
    ...
    function staff_details(){
        ...
        $page_data['active_link']  = 'staff_details';
        $page_data['page_title'] = get_phrase('staff_details');
        $this->load->view('backend/index', $page_data);
    }
    
    public function update_travelled_amount(){
        $this->db->where('id', $_POST['id']);
        $this->db->update('travelled', array('due_amount'=>$_POST['amount']));
        echo 1;die;
    }
