...
//die;
       ...
       // check_permission(ADD);
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                //print_r($data);die;
                if(!empty($id)){
                    $insert_id = $this->inventory->update('inventory_travel', $data,['id'=>$id]);
                } else {
        ...
        $this->data['add']         = TRUE;
        if(!empty($id)){
            $this->data['obj'] = $this->db->get_where('inventory_travel',['id'=>$id])->row();
        }
        $this->data['id'] = $id;
    ...
    private function _prepare_room_validation() {
     ...
     private function _get_posted_room_data() {
        ...
        if ($this->input->post('id')) {
            ...
            $data['cost'] = $this->input->post('amount');
    ...
    * ********************************************************** */
    ...
    public function delete_travel($id = null) {
        ...
        if(!is_numeric($id)){
           error($this->lang->line('unexpected_error'));
          redirect('transport/vehicle/list_travel');
        }
        
        $vehicle = $this->vehicle->get_single('inventory_travel', array('id' => $id));
        if ($this->vehicle->delete('inventory_travel', array('id' => $id))) {
            $this->session->set_flashdata('flash_message' , get_phrase('data_delete_successfully'));
        } else {
            $this->session->set_flashdata('error_message' , get_phrase('data_delete_failed'));
        }
