...
class Route extends MY_Controller {
    ...
    * ********************************************************** */
    public function index() {
       ...
       // $qqqq       = $this->route->get_list('routes', array('status' => 1));
        // $this->data['add_vehicles'] = $this->route->get_vehicle_list();
       /* foreach ($this->data['routes']  as $key => $dt) {
          echo "<pre>";
           print_r(json_decode($dt->stop_details));
          echo "</pre>";
        }
         */
         //echo  json_decode($qqqq[0]->stop_details);
        $this->data['list']         = TRUE;
		$this->data['page_name']    = 'index';
		$this->data['page_title']   = 'Route Type';
        $this->data['folder']       = 'transport/route';
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($insert_id) {
                   ...
                   $this->session->set_flashdata('error_message' , get_phrase('data_added_successfully'));
    ...
    * ********************************************************** */
    ...
    public function assign_vehicle(){
        ...
        $this->form_validation->set_rules('route', $this->lang->line('route'), 'trim|required');
        $this->form_validation->set_rules('vehicle', $this->lang->line('vehicle'), 'trim|required');
        if ($this->form_validation->run() === TRUE) {
            $this->db->where('id',$this->input->post('route'));
            $this->db->update('routes',array('vehicle_ids'=>$this->input->post('vehicle'))); 
            $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
        }
        redirect('transport/route');
    }
    
    public function assign_route_to_student(){
        $this->load->library('form_validation');
        $this->form_validation->set_error_delimiters('<div route="error-message" style="color: red;">', '</div>');
        $this->form_validation->set_rules('route', $this->lang->line('route'), 'trim|required');
        ...
        if ($this->form_validation->run() === TRUE) {
            ...
            $this->db->update('student',array('transport_id'=>$this->input->post('route') , 'is_transport_member'=>1));
            $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
        }
        redirect('transport/route');
    }
    
    public function un_assign_route_to_student(){
        $this->form_validation->set_rules('student_id', 'student_id', 'trim|required');   
         if ($this->form_validation->run() === TRUE) {
              $this->db->where('student_id',$this->input->post('student_id'));
            $this->db->update('student',array('transport_id'=>null , 'is_transport_member'=>0));
            $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
         }
         redirect('transport/route');
    }
    ...
    * ********************************************************** */
    public function view($id = null) {
       ...
       // check_permission(VIEW);
        ...
        $this->layout->view('route', $this->data);
    }
    ...
    public function get_single_route(){
       ...
       $this->load->view('transport/route/get-single-route', $this->data);
    }
    ...
    * ********************************************************** */
    private function _get_posted_route_data() {
        ...
        if ($this->input->post('id')) {
            $data['source_lat']  = $this->input->post('route_start_lat');
            $data['source_long'] = $this->input->post('route_start_lng');
            $data['dest_lat']    = $this->input->post('route_end_lat');
            $data['dest_long']   = $this->input->post('route_end_lng');
            // $data['stop_details']= json_encode($this->input->post('stop_point_lat'));
    ...
    function get_route_data($vehicle_number = ''){
        header('Content-Type: application/json');
        if(!empty($vehicle_number)){
            $vehicle_number = urldecode($vehicle_number);
            $drive_id = $this->db->get_where('vehicles',['number'=>$vehicle_number])->row()->driver;
            echo json_encode($this->db->get_where('vehicle_location',['driver_id'=>$drive_id])->row());
        }
        die;
    }
    
    function get_live_location($driver_id = ''){
        header('Content-Type: application/json');
        if(!empty($driver_id)){
            echo json_encode($this->db->get_where('vehicle_location',['driver_id'=>$driver_id])->row());
        }
        die;
    }
