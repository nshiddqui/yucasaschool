...
class Visitor extends MY_Controller {
    ...
    function __construct() {
        ...
        $this->data['roles_details'] = $this->visitor->get_list('roles', array('status' => 1), '', '', '', 'id', 'ASC');
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($updated) {
                    ...
                    $this->session->set_flashdata('flash_message', get_phrase('data_update_successful'));
                    redirect('visitor');
                } else {
