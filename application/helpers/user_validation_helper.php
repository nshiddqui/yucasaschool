...
if ( ! function_exists('duplication_of_section_on_create')){
	function duplication_of_section_on_create($class_id, $section_name,$sub_teacher){
		$ci=& get_instance();
		$num_rows = 0;
		$data = array(
		'class_id' => $class_id,
...
//parent email id duplication on insert
if ( ! function_exists('parent_email_validation')){
    function parent_email_validation($parent_email){
        $ci=& get_instance();
        $num_rows = 0;

        $num_rows = $ci->db->get_where('parent',array('email'=>$parent_email))->num_rows();
        if($num_rows == 0){
            return true;
        }
        else if($num_rows > 1){
